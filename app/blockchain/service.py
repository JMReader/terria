...
def _manifests_for(datasets: list[TimelapseDatasetSummary]) -> list[TimelapseManifest]:
    return [
        manifest
...
def _persist_and_anchor(
    *,
    ...
    """Guarda el snapshot canónico y, si corresponde, ancla su hash on-chain."""
    repo = _repo()
    cert_uid = snapshot["cert_uid"]
    payload = canonical_bytes(snapshot)
    content_hash_hex = sha256_hex(payload)

    record = repo.create_certification(
        field_id=field.id,
        version=version,
        cert_uid=cert_uid,
    ...
    repo.save_payload(record.id, payload, content_hash_hex)
    ...
    try:
        ...
        repo.enqueue_anchor_job(record.id, message)
        record = repo.set_status(record.id, "pending_anchor")
        return build_certification_response(record)
    ...
    record = repo.set_status(record.id, "anchored", issued_at=datetime.now(timezone.utc))
    return build_certification_response(record)
...
def issue_certification(
    *,
    ...
    repo = _repo()
    cert_uid = str(uuid4())
    version = repo.next_version(field.id)
    prev_hash = repo.previous_content_hash(field.id)

    manifests = _manifests_for(datasets)
    ...
    return _persist_and_anchor(
        field=field, snapshot=snapshot, version=version, prev_hash=prev_hash, anchor=anchor
    )


def monthly_cert_uid(field_id: UUID, month: str) -> str:
    """cert_uid determinístico por (campo, mes): re-emitir el mismo mes es no-op."""
    return str(uuid5(NAMESPACE_URL, f"terria:monthly:{field_id}:{month}"))


def issue_monthly_certifications(
    *,
    field: FieldResponse,
    datasets: list[TimelapseDatasetSummary],
    anchor: bool = True,
) -> list[CertificationResponse]:
    """Emite la certificación viva: un snapshot acumulado por mes, encadenado.

    Cada versión incluye todo el historial hasta ese mes (`observations` +
    `monthly`) y encadena `prev_content_hash` con la versión anterior del campo.
    Idempotente: los meses ya emitidos se omiten por `cert_uid` determinístico.
    """
    repo = _repo()
    manifests = _manifests_for(datasets)
    observations = build_observations(manifests)
    ...
    issued: list[CertificationResponse] = []
    for summary in monthly:
        month = summary["month"]
        cert_uid = monthly_cert_uid(field.id, month)
        if repo.get_by_cert_uid(cert_uid) is not None:
            continue

        cumulative_monthly = monthly_up_to(monthly, month)
        cumulative_observations = observations_up_to(observations, month)
        version = repo.next_version(field.id)
        prev_hash = repo.previous_content_hash(field.id)
        snapshot = build_snapshot(
            field=field,
            datasets=datasets,
            period_from=int(cumulative_monthly[0]["month"][:4]),
            period_to=int(month[:4]),
            cert_uid=cert_uid,
            schema_version=settings.cert_schema_version,
            prev_hash=prev_hash,
            observations=cumulative_observations,
            monthly=cumulative_monthly,
            sources=sources,
            scope="month",
            month=month,
        )
